...
const List = (props) => {
    ...
    const [editjob,seteditjob] = useState(props.job)
...
const checkhandler = () => {
  props.checklist(props.id);
    ...
    const editjobhandler = (event) => {
        seteditjob(event.target.value);
        
    }

    const updatehandler = (event) => {
        if (event.key == "Esc" || event.key == "Enter") {
            props.editlist(props.id, editjob);
           setedit(false);
        }
    }
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            {edit ? (
              <input
                type="text"
                              className="border-2 border-zinc-500 focus-visible:outline-none flex-grow px-3"
                              value={editjob}
                              onChange={editjobhandler}
          ...
          <div className="flex gap-2 duration-300 translate-x-[120%] group-hover:translate-x-0">
            <button onClick={editbtnhandler} className="list-edit-btn border duration-200 active:scale-90 border-zinc-700 p-2">
              <svg
            ...
            <button onClick={delbtnhandler} className="list-del-btn border duration-200 active:scale-90 border-zinc-700 p-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
